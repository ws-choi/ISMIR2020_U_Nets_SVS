...
def get_loss(loss_name, window_length, hop_length, **kwargs):
    ...
    if loss_name == 'spec_l1':
        return Spectrogram_Loss('l1')
    elif loss_name == 'spec_l2':
        return Spectrogram_Loss('l2')
    elif loss_name == 'spec_mse':
        return Spectrogram_Loss('mse')
        ...
        return Raw_And_Spec(**kwargs)

    elif loss_name == 'distortion':
        return Distortion_Loss(window_length, hop_length)
    elif loss_name == 'sdr_like':
        return SDR_LIKE_Loss(window_length, hop_length)
    elif loss_name == 'sdr':
        return SDR(window_length, hop_length)
    elif loss_name == 'dsr':
        return DSR(window_length, hop_length)
    elif loss_name == 'nsdr':
        return NSDR(window_length, hop_length)
    elif loss_name == 'nsdr_44100':
        return NSDR(44100, 22050)
    elif loss_name == 'ldsr':
        return LDSR(window_length, hop_length)
    elif loss_name == 'ncs':
        return NCS_Loss(window_length, hop_length)
    elif loss_name == 'ncs_44100':
        return NCS_Loss(44100, 22050)
    elif loss_name == 'nlcs':
        return NLCS_Loss(window_length, hop_length)
...
class Spectrogram_Loss(Loss):
    ...
    def compute(self, model, mixture_signal, target_signal):
        ...
        target_hat, target_spec = model.forward_with_target_spec(mixture_signal, target_signal)
        return self.criterion(target_hat, target_spec)
...
class Raw_And_Spec(Loss):
    ...
    def compute(self, model, mixture_signal, target_signal):
        ...
        # raw and spec
        target_signal_hat, target_spec_hat = model.separate_and_return_spec(mixture_signal)
...
class Distortion_Loss(Unfolding_Loss):
...
class NLCS_Loss(Unfolding_Loss):
    def __init__(self, window_length, hop_length, **kwargs):
        super().__init__(window_length, hop_length)
