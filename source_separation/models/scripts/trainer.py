...
def train(param):
    ...
    if args['last_activation'] != 'identity' and args['spec_est_mode'] != 'masking':
        warn('Please check if you really want to use a mapping-based spectrogram estimation method '
             'with a final activation funtion. ')
    ##########################################################
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        verbose=False,
        monitor='val_loss',
        prefix='{}_'.format(args['target_name']),
        save_last=False,
    ...
    if args['dev_mode']:
        ...
        args['logger'] = WandbLogger(project='one2one_separation', tags=args['model'], offline=False, id=run_id)
